package io.Rafa_lol.full_Project.exception;
...
public class HandleException extends ResponseEntityExceptionHandler implements ErrorController {
    ...
    public ResponseEntity<HttpResponse> accessDeniedException(AccessDeniedException exception) {
        ...
        return new ResponseEntity<>(
                HttpResponse.builder()
                        .timestamp(now().toString())
                        .reason("Access denied. You do not have access")
                        .developerMessage(exception.getMessage())
                        .status(FORBIDDEN)
        ...
        return new ResponseEntity<>(
                HttpResponse.builder()
                        ...
                        .reason(exception.getMessage() != null ?
                                (exception.getMessage().contains("expected 1, atcual 0") ? "Record not found" : exception.getMessage())
                                        : "Some error corrured")
                        .developerMessage(exception.getMessage())
    ...
    public ResponseEntity<HttpResponse> emptyResultDataAccessException(EmptyResultDataAccessException exception) {
        ...
        return new ResponseEntity<>(
                HttpResponse.builder()
                        .timestamp(now().toString())
                        .reason(exception.getMessage().contains("expected 1, atcual 0") ? "Record not found" : exception.getMessage())
                        .developerMessage(exception.getMessage())
                        .status(BAD_REQUEST)
                        .statusCode(BAD_REQUEST.value())
                        .build(), BAD_REQUEST);
